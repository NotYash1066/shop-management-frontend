...
export function EmployeesPage() {
  ...
  const saveMutation = useMutation({
    mutationFn: async () => {
      if (editingId) {
        return api.updateEmployee(token, editingId, draft);
      }
      return api.createEmployee(token, draft);
    },
    onSuccess: () => {
  ...
  const deleteMutation = useMutation({
    mutationFn: (id: number) => api.deleteEmployee(token, id),
  ...
  return (
    <div className="space-y-10">
      <PageIntro
        eyebrow="Employees"
        title="Staff records without dashboard theater."
        description="Employees are reference data here, not a social app. The layout stays compact, structured, and operational."
      />
      <section className="section-frame grid overflow-hidden xl:grid-cols-[1.25fr_0.75fr]">
        <div>
          <div className="grid grid-cols-[0.85fr_0.85fr_1fr_auto] border-b border-line px-6 py-4 text-[11px] font-semibold uppercase tracking-[0.32em] text-muted sm:px-8">
            <span>Name</span>
            <span>Role</span>
            <span>Email</span>
          ...
          {(employeesQuery.data || []).map((employee) => (
            <div
              key={employee.id}
              className="ledger-row grid grid-cols-[0.85fr_0.85fr_1fr_auto] items-center gap-4 border-b border-line px-6 py-5 transition hover:bg-white/35 sm:px-8"
            >
          ...
          <div className="space-y-5 border-t border-white/10 px-6 py-6 sm:px-8">
            ...
            {canWrite ? (
              ...
              <div className="border-l-2 border-white/40 bg-white/5 px-4 py-4 text-sm text-white/70">
                Employee modifications require admin access.
              </div>
            )}
          </div>
        </aside>
